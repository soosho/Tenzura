...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for TENZ
    ...
    if (p_rewardSnapshot.strDistributionAsset != "TENZ") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is TENZ with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using TENZ differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "TENZ") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Tenzura address (already validated during ownership snapshot creation)
