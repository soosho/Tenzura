...
void static TenzuraMiner(const CChainParams& chainparams)
{
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("TenzuraMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("TenzuraMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("TenzuraMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("TenzuraMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("TenzuraMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateTenzuras(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&TenzuraMiner, boost::cref(chainparams)));
    }
