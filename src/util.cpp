...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Tenzura
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Tenzura
    // Mac: ~/Library/Application Support/Tenzura
    // Unix: ~/.tenzura
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tenzura";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tenzura";
#else
    // Unix
    return pathRet / ".tenzura";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string &confPath)
{
    ...
    if (!streamConfig.good())
        return; // No tenzura.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override tenzura.conf
