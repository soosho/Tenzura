...
The Tenzura Core daemon can be configured to act as a trusted "border
router", implementing the tenzura wire protocol and relay, making
...
The ZeroMQ feature in Tenzura Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building tenzurad:
...
For instance:
    ...
    $ tenzurad -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/tenzurad.tx.raw
...
These options can also be provided in tenzura.conf.
...
From the perspective of tenzurad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into tenzurad directly. Furthermore, no information is
...
using. Tenzurad appends an up-counting sequence number to each
